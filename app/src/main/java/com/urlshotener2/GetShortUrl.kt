...
import com.urlshotener2.tool.getDate
import com.urlshotener2.tool.getTime
...
fun getShortUrlFromApi(context: Context, longUrl: String, onSuccess: (String) -> Unit, onFailure: (String) -> Unit) {
    val queue = Volley.newRequestQueue(context)

    val stringRequest = StringRequest(
        Request.Method.GET, BASE_URL + longUrl,
        { response ->
            onSuccess(response)
        },
        { error ->
            onFailure(error.localizedMessage ?: "Unknown error")
        }
    )
